...
setTimeout(function () {
  console.log('children2', CHILDREN, CHILDREN.length);
}, 100);
...
var SingleChildProto = {
  ...
  restart: function (cb) {
    ...
    this.stop(function killedChild (code) {
      ...
      var child = spawn(that.cmd, that.args);
      that.child = child;
...
console.log('killing time', CHILDREN.length);
  nimble.each(CHILDREN, function killChild (child, cb) {
    console.log('abc', child.pid);
    // TODO: Try out taskkill https://github.com/remy/nodemon/blob/master/nodemon.js#L405
