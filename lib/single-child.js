...
function killChild(child, signal, cb) {
  // If there is a callback, run it on exit
  if (cb) {
    child.on('exit', cb);
  }
...
function SingleChild(cmd, args, options) {
  ...
  var that = this;
  CHILDREN.push(that);
  console.log('here', CHILDREN.length);
...
var SingleChildProto = {
  ...
  kill: function (options, cb) {
    ...
    var killSignal = options.signal || this.options.killSignal,
        that = this;
    killChild(child, killSignal, function cleanupChild (code, signal) {
      // Unset the child
      that.child = null;
...
function slaughterChildren () {
  nimble.each(CHILDREN, function killEachChild (child, cb) {
    // Remove all listeners to prevent hanging
    child.removeAllListeners('exit');

    // Kill the child
    killChild(child, null, function (err) {
      cb();
    });
